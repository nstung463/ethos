"""SkillsMiddleware — scans skills/ directory once per session and injects metadata into the system prompt.

Pattern mirrors deepagents SkillsMiddleware with progressive disclosure:
- before_agent scans skill directories, parses YAML frontmatter, stores metadata in state
- modify_request injects name+description+path into the system prompt
- The agent reads the full SKILL.md via read_file when it actually needs to use a skill
- PrivateStateAttr marks skills_metadata as private (subagents load their own skills)
"""
...
class SkillsState(AgentState):
    ...
    Marked Private so each subagent loads its own skills independently.
    """
...
def _parse_skill(skill_md: Path) -> SkillMetadata | None:
    ...
    try:
        ...
        logger.warning("YAML error in %s: %s", skill_md, e)
        return None
...
def _scan_skills(skills_dir: str) -> list[SkillMetadata]:
    ...
    for subdir in sorted(root.iterdir()):
        ...
        if metadata:
            skills[metadata["name"]] = metadata  # last one wins for duplicates
...
class SkillsMiddleware(AgentMiddleware[SkillsState, ContextT, ResponseT]):
    ...
    Placement: put this before MemoryMiddleware so skills appear before memory
    ...
    def before_agent(  # type: ignore[override]
        ...
        """Scan skills directory and store metadata (runs once; skips if already loaded)."""
